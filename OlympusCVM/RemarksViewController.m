...
//        NSString *url = [NSString stringWithFormat:@"%@/api/v1/%@",[self.superCate lowercaseString]];
        ...
        if ([[userData valueForKey:@"is_testing"] boolValue]) {
            url = [NSString stringWithFormat:@"%@/api/v1/service",[userData valueForKey:@"testing_url"]];
        }else{
            url = [NSString stringWithFormat:@"%@/api/v1/service",base_url];
        }
...
-(void)fetchCountForLoginUser{
    if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"isLogin"] boolValue]) {
        ...
        if ([[userData valueForKey:@"is_testing"] boolValue]) {
            ...
            url = [NSString stringWithFormat:@"%@/api/v1/historyCount/%@?auth_token=%@",base_url,[[[UtilsManager sharedObject] getUserDetailsFromDefaultUser] valueForKey:@"id"],Auth_Token];
        }
