    ...
    if (mobile.text.length >6) {
        ...
        if ([[UtilsManager sharedObject] isValidPhone:mobNum]) {
            ...
            if ([[userData valueForKey:@"is_testing"] boolValue]) {
                url = [NSString stringWithFormat:@"%@/api/v1/customer/send_otp",[userData valueForKey:@"testing_url"]];
            }else{
                url = [NSString stringWithFormat:@"%@/api/v1/customer/send_otp",base_url];
            }
            ...
            [manager POST:url parameters:param progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                ...
                }else if ([[responseDict valueForKey:@"status"] intValue] == 403) {
