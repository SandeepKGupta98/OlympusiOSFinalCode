...
-(void)getRequestData{
    ...
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        url = [NSString stringWithFormat:@"%@/api/v1/service/%@?auth_token=%@",[userData valueForKey:@"testing_url"],self.reqId,Auth_Token];
    }else{
        url = [NSString stringWithFormat:@"%@/api/v1/service/%@?auth_token=%@",base_url,self.reqId,Auth_Token];
    }
