...
-(void)getRequestHistoryFromServer{
    ...
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        url = [NSString stringWithFormat:@"%@/api/v1/historyCount/%@?auth_token=%@",[userData valueForKey:@"testing_url"],[userInfo valueForKey:@"id"],Auth_Token];
    }else{
        url = [NSString stringWithFormat:@"%@/api/v1/historyCount/%@?auth_token=%@",base_url,[userInfo valueForKey:@"id"],Auth_Token];
    }
