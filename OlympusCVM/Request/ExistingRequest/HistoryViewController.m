...
//    [param setObject:[self.type lowercaseString] forKey:@"request_type"];
    ...
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        url = [NSString stringWithFormat:@"%@/api/v2/getRequestsHistory",[userData valueForKey:@"testing_url"]];
    }else{
        url = [NSString stringWithFormat:@"%@/api/v2/getRequestsHistory",base_url];
    }
...
-(void)sendAcknowledgementToServer:(NSString *)requestId{
    if (![[UtilsManager sharedObject] checkUserActivityValid]){
        [[UtilsManager sharedObject] sessionExpirePopup:self];
        return;
    }
    ...
    [param setObject:Auth_Token_New forKey:Auth_Token_KEY];
//    [param setObject:[self.type lowercaseString] forKey:@"request_type"];
    [param setObject:requestId forKey:@"request_id"];//1626
    [param setObject:[NSNumber numberWithInt:1] forKey:@"acknowledgement_status"];//1626
    ...
    [manager.requestSerializer setValue:[NSString stringWithFormat:@"Bearer %@",access_token] forHTTPHeaderField:@"Authorization"];
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        url = [NSString stringWithFormat:@"%@/api/v2/request-acknowledge",[userData valueForKey:@"testing_url"]];
    }else{
        url = [NSString stringWithFormat:@"%@/api/v2/request-acknowledge",base_url];
    }
    ...
    [manager POST:url parameters:param progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([[responseDict valueForKey:@"status_code"] intValue] == 200) {
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"MY VOICE" message:[responseDict valueForKey:@"message"] preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *ok = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:nil];
            [alert addAction:ok];
            [self presentViewController:alert animated:YES completion:nil];
        ...
        if (self.segmentCntrl.selectedSegmentIndex == 0 ) {
            if ([[flagAry objectAtIndex:indexPath.row] boolValue]) {
                ...
                if ([[[reqInfo valueForKey:@"request_type"]lowercaseString] isEqualToString:@"service"]) {
                    if ([status.lowercaseString isEqualToString:@"dispatched"]){
                        return 218;
                    }else{
                        return 183;//200
                    }
...
//            cell.requestNo.text = [NSString stringWithFormat:@"%@ : Request No %@",[[reqInfo valueForKey:@"request_type"] capitalizedString],[[[reqInfo valueForKey:@"cvm_id"] componentsSeparatedByString:@"/"] lastObject]];
        ...
        if (![[reqInfo valueForKey:@"status"] isKindOfClass:[NSNull class]]) {
            ...
            if ([[[reqInfo valueForKey:@"request_type"]lowercaseString] isEqualToString:@"service"]) {
                if ([status.lowercaseString isEqualToString:@"dispatched"]){
                    cell.acknowledgementHgt.constant = 35;
                }else{
                    cell.acknowledgementHgt.constant = 2;
                }
            }else{
                cell.acknowledgementHgt.constant = 2;
            }
...
- (void)akcknowledgeButtonTappedWithCell:(HistoryCell *)cell andIndex:(NSInteger)index{
    NSDictionary *reqInfo = [historyAry objectAtIndex:index];
    if ([[[reqInfo valueForKey:@"request_type"]lowercaseString] isEqualToString:@"service"]) {
        NSString *status = [[reqInfo valueForKey:@"status"] stringByReplacingOccurrencesOfString:@"_" withString:@" "];
        if ([status.lowercaseString isEqualToString:@"dispatched"]){
            NSLog(@"Call akcknowledge API:- %@",reqInfo);
            if(![[reqInfo valueForKey:@"cvm_id"] isKindOfClass:[NSNull class]]){
                [self sendAcknowledgementToServer:[reqInfo valueForKey:@"cvm_id"]];
            }
...
-(void)escalationButtonTappedWithCell:(HistoryCell *)cell andIndex:(NSInteger)index{
    ...
    if ([[[historyAry objectAtIndex:index] valueForKey:@"is_escalated"] intValue] == 1) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Request Escalation" message:@"Your request has already been escalated. Please allow us some time to resolve the issue before you escalate again." preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)akcknowledgeBtnTapped:(id)sender {
    if (self.delegate) {
        [self.delegate akcknowledgeButtonTappedWithCell:self andIndex:self.index];
    }
}
