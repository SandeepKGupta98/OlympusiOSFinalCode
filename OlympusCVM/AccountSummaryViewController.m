...
- (IBAction)logoutButtonTapped:(id)sender{
    ...
    UIAlertAction *yes = [UIAlertAction actionWithTitle:@"YES" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if ([[userData valueForKey:@"is_testing"] boolValue]) {
            url = [NSString stringWithFormat:@"%@/api/v1/customer/logout",[userData valueForKey:@"testing_url"]];
        }else{
            url = [NSString stringWithFormat:@"%@/api/v1/customer/logout",base_url];
        }
...
-(void)registerUsertoServer{
    ...
    NSDictionary *userData = [[UtilsManager sharedObject] getUserDetailsFromDefaultUser];
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        url = [NSString stringWithFormat:@"%@/api/v1/customer",[userData valueForKey:@"testing_url"]];
    }else{
        url = [NSString stringWithFormat:@"%@/api/v1/customer",base_url];
    }
...
-(void)updateUserData{
    ...
    NSDictionary *userData = [[UtilsManager sharedObject] getUserDetailsFromDefaultUser];
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        apiurl = [NSString stringWithFormat:@"%@/api/v1/customer/%@",[userData valueForKey:@"testing_url"],[self.userDict valueForKey:@"id"]];
    }else{
        apiurl = [NSString stringWithFormat:@"%@/api/v1/customer/%@",base_url,[self.userDict valueForKey:@"id"]];
    }
...
//            [self.navigationController pushViewController:otpVC animated:NO];
            ...
            [[UtilsManager sharedObject] storeUserDatatoDefaultUser:[responseDict valueForKey:@"data"]];
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"" message:[responseDict valueForKey:@"message"] preferredStyle:UIAlertControllerStyleAlert];
