...
-(void)fetchCountForLoginUser{
    ...
    if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"isLogin"] boolValue]) {
        ...
        NSString *url = [NSString stringWithFormat:@"%@/api/v1/historyCount/%@?auth_token=%@",base_url,[[[UtilsManager sharedObject] getUserDetailsFromDefaultUser] valueForKey:@"id"],Auth_Token_New];
        ...
        if ([[userData valueForKey:@"is_testing"] boolValue]) {
            url = [NSString stringWithFormat:@"%@/api/v1/historyCount/%@?auth_token=%@",[userData valueForKey:@"testing_url"],[[[UtilsManager sharedObject] getUserDetailsFromDefaultUser] valueForKey:@"id"],Auth_Token_New];
        }
...
//            [[UtilsManager sharedObject] setInboxBadgeWithFullInboxCount:inboxcount];
...
- (IBAction)serviceButtonTapped:(id)sender {
    ...
    if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"isLogin"] boolValue]) {
//        PopupViewController *popupVC = [[PopupViewController alloc] initWithNib:@"PopupViewController" ];
//        //popupVC.iconImg.image = [UIImage imageNamed:@"y1.png"];
//        popupVC.descLbl.text = @"Now you can directly fix a service for your equipment using the “My Voice” App. Simply click on the ‘Service’ button to initiate the process.";
