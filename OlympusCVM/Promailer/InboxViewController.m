...
-(void)getInboxFromServer{
    ...
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        apiurl = [NSString stringWithFormat:@"%@/api/v2/promailersLatest",[userData valueForKey:@"testing_url"]];
    }else{
        apiurl = [NSString stringWithFormat:@"%@/api/v2/promailersLatest",base_url];
    }
...
//            historyInfo = [NSDictionary dictionaryWithDictionary:[responseDict valueForKey:@"data"]];
            ...
            self.inboxCount.text = [NSString stringWithFormat:@"INBOX(%@)",[responseDict valueForKey:@"count"]];//(long)self->inboxAry.count

//            [[UtilsManager sharedObject] inboxBadgeWithInbox:inboxAry];
            [[self.tabBarController.tabBar.items objectAtIndex:3] setBadgeValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:@"count"]]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[inboxDict valueForKey:@"is_read"] boolValue]){
