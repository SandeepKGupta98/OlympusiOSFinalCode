...
- (void)viewDidLoad {
    ...
    if (self.hospitalIndex == nil){
        self.hospitalIndex = -2344;
    }
...
-(void)setHospitalDataInField{
    ...
    if (deptAry == nil){
        if ([self.hosInfo valueForKey:@"dept_id"] != nil){
            
            NSArray *idAry = [[[self.hosInfo valueForKey:@"dept_id"] stringByReplacingOccurrencesOfString:@" " withString:@""] componentsSeparatedByString:@","];
            for (int i=0; i<idAry.count; i++) {
                for (NSDictionary *dict in  depAry){
                    if ([[NSString stringWithFormat:@"%@",[dict valueForKey:@"id"]] isEqualToString:[NSString stringWithFormat:@"%@",[idAry objectAtIndex:i]]]){
                        [selectDepAry addObject:dict];
                        break;
                    }
                }
            }
...
-(void)fetchDepartmentsFromServer{
    ...
    if ([[userData valueForKey:@"is_testing"] boolValue]) {
        apiurl = [NSString stringWithFormat:@"%@/api/v2/departments?auth_token=%@",[userData valueForKey:@"testing_url"],Auth_Token];
    }else{
        apiurl = [NSString stringWithFormat:@"%@/api/v2/departments?auth_token=%@",base_url,Auth_Token];
    }
    ...
    [manager GET:apiurl parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        self->depAry = [NSMutableArray arrayWithArray:responseObject];
...
-(IBAction)footerDoneBtnTapped:(id)sender{
    ...
    if (selectDepAry.count == 0){
        [self showAlertWithMessage:@"Please select atleast one department."];
        return;
    }
...
//    selectDepAry
    ...
    NSMutableSet *tempAry = [[NSMutableSet alloc] init];
    for (int i = 0; i < depAry.count; i++) {
        NSDictionary * d = depAry[i];
        NSString *name = [d valueForKey:@"name"];
        NSArray *dep = [depName.text componentsSeparatedByString:@","];
        for (int j = 0; j < dep.count; j++) {
            NSString *tempDep = dep[j];
            if ([[tempDep stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString: name]){
                [tempAry addObject:d];
            }
        }
    }
    NSMutableArray *selDep = [[NSMutableArray alloc] initWithArray:[tempAry allObjects]];
    
    if (selDep.count < [[depName.text componentsSeparatedByString:@","] count]){
        [selDep addObject:[depAry lastObject]];
    }
    
    
    
    
    NSLog(@"Selected Ary: %@", selDep);
    return selDep;
}
...
//            [optionAry addObject:@"Others"];
            ...
            selectDepAry = [self getSelectedData];
            [self.tableView reloadData];
...
//    UITextField *othrCountry;
    ...
    if (depName.text.length == 0) {
        [self showAlertWithMessage:@"Please enter department name."];
        return;
    ...
    if (zip.text.length == 0) {
        ...
        return;
    }else if(zip.text.length != 6){
        [self showAlertWithMessage:@"Invalid zip code. Please enter 6 digits zip code."];
        return;
    }else{
    ...
    NSLog(@"hospitalsVC.hospitalIndex %ld",self.hospitalIndex);
    [self dismissViewControllerAnimated:NO completion:^{
        if (self.delegate) {
            [self.delegate dismissHospitalVCWithInfo:self.hosInfo index:self.hospitalIndex];
        }
    }];
...
-(void)awakeFromNib{
    ...
    UILabel *placeholderLabel = object_getIvar(self.depTextField, ivar);
