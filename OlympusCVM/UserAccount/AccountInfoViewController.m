...
//    lName.attributedPlaceholder = [self getMutableStringWithString:@"Last Name" font:TextField_Font spacing:0 textColor:[UIColor lightGrayColor] lineSpacing:0 andNSTextAlignment:NSTextAlignmentLeft];
    ...
    mobile = [[UITextField alloc] initWithFrame:CGRectMake(45, y, screen_wdth-80, 40)];
//    mobile.attributedPlaceholder = [self getMutableStringWithString:@"Mobile Number" font:TextField_Font spacing:0 textColor:[UIColor lightGrayColor] lineSpacing:0 andNSTextAlignment:NSTextAlignmentLeft];
    ...
    UILabel *mobileLeftlbl = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 30, 40)];
    mobileLeftlbl.textAlignment = NSTextAlignmentCenter;
    mobileLeftlbl.font = TextField_Font;
    mobileLeftlbl.text = @"+91";
//    mobileLeftlbl.backgroundColor = [UIColor cyanColor];
    mobileLeftlbl.textColor = [UIColor whiteColor];
//    tempView.backgroundColor = [UIColor yellowColor];
    [tempView addSubview:mobileLeftlbl];
    mobile.leftView = tempView;
    mobile.autocorrectionType = UITextAutocorrectionTypeNo;
...
//    cEmail.attributedPlaceholder = [self getMutableStringWithString:@"Confirm Email" font:TextField_Font spacing:0 textColor:[UIColor lightGrayColor] lineSpacing:0 andNSTextAlignment:NSTextAlignmentLeft];
    ...
    cEmail.autocorrectionType = UITextAutocorrectionTypeNo;
    cEmail.autocapitalizationType = UITextAutocapitalizationTypeNone;
    ...
    if ([self.userType isEqualToString:@"new"]) {
        ...
        cPwd.autocorrectionType = UITextAutocorrectionTypeNo;
...
//    tncTextView.attributedText = str;
    ...
    hosName.autocorrectionType = UITextAutocorrectionTypeNo;
...
-(void)dismissHospitalVCWithInfo:(NSMutableDictionary *)infoDict index:(NSInteger) hosIndex{
    if (infoDict.allKeys.count>0) {
        ...
        }else if((hosIndex-999) >= 0){
            [hospitalAry replaceObjectAtIndex:(hosIndex-999) withObject:infoDict];
        }else{
...
-(void)editHospitalBtnTapped:(UIButton *)btn{
    ...
    UIAlertAction *yes = [UIAlertAction actionWithTitle:@"YES" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        HospitalsViewController *hospitalsVC = [mainStoryboard instantiateViewControllerWithIdentifier:@"hospitalsVC"];
        hospitalsVC.delegate = self;
        hospitalsVC.hospitalIndex = index+999;
        hospitalsVC.hosIndex = [NSNumber numberWithLong:index];
        hospitalsVC.hosInfo = [NSMutableDictionary dictionaryWithDictionary:hosInfo];
        hospitalsVC.modalPresentationStyle = UIModalPresentationFullScreen;
        [self presentViewController:hospitalsVC animated:NO completion:nil];
        NSLog(@"hospitalsVC hospitalIndex %ld",hospitalsVC.hospitalIndex);
...
//        NSString *subStrFrom =  [mobile.text substringFromIndex:3];
    ...
    if (cEmail.text.length == 0) {
        [self showAlertWithMsg:@"Confirm email cannot be blank."];   return;
    }
    if (![[UtilsManager sharedObject] isValidEmail:cEmail.text]) {
        [self showAlertWithMsg:@"Invalid confirm email."];   return;
    }
    if (![email.text isEqualToString:cEmail.text]) {
...
//    }
    ...
    if ([self.userType isEqualToString:@"new"]) {
        ...
        if (![[UtilsManager sharedObject] isValidPassword:pwd.text]){
        ...
        if ([[UtilsManager sharedObject] isPasswordContainingSequence:cPwd.text]) {
...
-(void)checkPwdValidation{
    ...
    [param setObject:[self.userInfo valueForKey:@"email"] forKey:@"email"];
    [param setObject:[self.userInfo valueForKey:@"password"] forKey:@"password"];
    [param setObject:[self.userInfo valueForKey:@"mobile_number"] forKey:@"mobile_number"];
    [param setObject:[self.userInfo valueForKey:@"first_name"] forKey:@"first_name"];
    [param setObject:[self.userInfo valueForKey:@"last_name"] forKey:@"last_name"];
    [param setObject:[self.userInfo valueForKey:@"mobile_number"] forKey:@"mobile_number"];
...
//        self.loaderView.hidden = YES;
        ...
        if ([[responseDict valueForKey:@"status_code"] intValue] == 200) {
            if (self->hospitalAry.count>0) {
                [self.userInfo setObject:self->hospitalAry forKey:HOSARY_KEY];
            }else{
                [self showAlertWithMsg:@"Please add atleast one hospital."];   return;
            }
            if (![[[NSUserDefaults standardUserDefaults] valueForKey:@"isLogin"] boolValue]) {
